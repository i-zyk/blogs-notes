...
```
```
// yd-mastra-app/.env.devlopment

OPENAI_API_KEY=your-api-key

然后把AIAgent接到client上

MCP（Model Context Pro）画3D模型

让 Blender 自动画3D模型-启动MCP Server

mcp.so网站找mcp

找到后在claude的seting的Developer上添加服务配置

腾讯混元3D免费生成3D纹理

icraf.design 画3D架构设计图，没有的icon可以用mcp画好后上传
...
claude：github MCP的链接，我使用了这个 mcp server请告诉我一下有多少指令，并用中文告诉我怎么使用

创建一个名字叫yideng仓库 public的 然后里面新建一个基础的koa项目

创建一个pr，主要是对app.js 进行更完善的设计，以后别人提的merge就可以通过ai帮你判断是否合入


利用该MCP创建一个PR

1. 完成一个AI CR Agents
2. 选1-2个mcp.so 能够帮助自己解决实际问题，新奇的mcp服务
```
...
localStorage 存在性能问题，最大存储5MB，有些手机用了3M左右就开始卡，用一半放在indexDB「异步的，非关系性的，IO，这个性能也差，以前不好使，现在可以使了」，使用扩容1个域名5M，多使用几个域名，放几个iframe，和iframe交互
```
