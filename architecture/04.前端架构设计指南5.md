...
## immer
 ...
 export type ImmerHooks<S> = [S, Update<S>];

 //ts函数签名 接收initialValue参数 类型是S 或者 () => S 返回 [state, setState]
 export function useImmer<S = unknown>(initialValue: S 
 | (() => S)) : ImmerHooks<S>;
 ...
 ```
  ...
  const App = () => {
    const [data, setData] = useImmer({info: 'xxxx'});
