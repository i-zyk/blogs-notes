...
pnpm run client:dev // src/index.js 写js直接编译，ts报错

有很多包会破坏webpack的prepack，选tsload我们选swc
  esbuild【go写的会破坏】,
  swc【rust写的不会破坏】
如vite【用的esbuild，体积很小，没有prepack】
...
1. 自己能够使用rust go awasm 写loader 和 plugin
<!-- webpack.config.js -->
...
//.swcrc「swc的配置」
{
  ...
  "jsc": {
		...
		"transform": {
			...
			"react": {
				"runtime": "automatic", //以前需要import导入，现在不用了
				"refresh": true,
				"development": false,
...
编译后，需要dist/index.html 手动引入main.js太麻烦，安装html-webpack-plugin 后就可以自动插入了

然后main.js每次都会变化，需要在安装clean-webpack-plugin，有变化的话会自动清理了
```
...
// 调整开发环境的output，使js文件放入dist/scripts下
...
1. 安装无头浏览器 配置engineOptions

2. 修改截图尺寸

3. UI设计稿放到 新建的 bitmaps_reference 文件夹中 进行对比
...
(async function example() {
  // 修改成与下载所对应无头浏览器名 Browser.FIREFOX
...
// webpack自带的一些工具，避免和自己的eslint发生冲突
...
// 安装7.0的老版本，也可以安装最新的，写法不一样，看个人喜好
...
把pack.json 拖到AI，让AI帮忙写一份README.md
...
无意义渲染例子：两次的状态一样的时候，react也会重新渲染
pages/app.tsx
```
